...
class MetaGeoEvaluator:
    ...
    "base_url": "repalce with your base url"
    }
    ...
    def __init__(self, args):
        ...
        self.evaluation_data = self._prepare_evaluation_data()[:20]
    ...
    def _extract_related_content(self, predict: str, name: str, content: str) -> str:
        """调用GPT-4o提取响应中相关的内容"""
        prompt = (
            f"You are a geometry problem grader. Extract relevant content from the model's response based on the following knowledge point:\n"
            f"Knowledge Point Name: '{name}'\nKnowledge Point Content: '{content}'\n\n"
            f"Model Response: '{predict}'\n\n"
            "Please directly output the parts of the model's response related to this knowledge point"
        )
    ...
    def _evaluate_alignment_accuracy(self, response: str, ground_truth: str) -> Dict:
        """评估对应准确性"""
        prompt = (
            "You are a geometry problem grader. Evaluate the model's description against the standard answer's knowledge point.\n"
            "Specifically, in the standard answer, content wrapped in <note></note> are key points. You need to determine:\n"
            "- num_exist: the number of key points (from the standard answer) present in the model's response (regardless of correctness)\n"
            "- num_acc: the number of key points both present and correctly corresponding\n"
            "- ground_total: the total key points in standard answer's <note></note> tags\n\n"
            "Note: Treat angles like ABC and CBA as equivalent. Output format: [ans]num_exist, num_acc, ground_total[/ans]\n\n"
            f"Standard description: '{ground_truth}'\n"
            f"Model's related steps: {response}"
        )
    ...
    def _build_prompt(predict: str, answer: str) -> str:
        """构建评估提示模板"""
        return (
                f"Given the model's prediction: '{predict}' and the correct answer: '{answer}', please determine if the model's predicted answer is correct. Please respond with either [[ANSWER]]Yes[[/ANSWER]] or [[ANSWER]]No[[/ANSWER]]."
            )
    ...
    def _build_knowledge_prompt(predict: str, name: str, content: str) -> str:
        """构建知识点评测提示模板"""
        return (
            "Strictly judge whether the model's predicted response uses the following geometry knowledge point:\n"
            f"Knowledge Point Name: {name}\nKnowledge Point Content: {content}\n\n"
            f"Model Response: '{predict}'\n\n"
            "Please respond with either [[ANSWER]]Yes[[/ANSWER]] or [[ANSWER]]No[[/ANSWER]]."
        )
...
def parse_response(response_text: str) -> bool:
    """解析模型响应"""
    match = re.search(r"\[\[answer\]\](yes|no)\[\[/answer\]\]", response_text)
    match = re.search(r'\[\[answer\]\](yes|no)\[\[/answer\]\]', response_text)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--generate_texts_path",
        type=str,
        default="../resoning_results_en/Qwen2.5-VL-3B-Instruct_output.jsonl",
        help="模型生成结果文件路径"
    )
    parser.add_argument(
        "--model_name",
    ...
    parser.add_argument(
        "--ground_truth_path",
        type=str,
        default="../data/full_data_en.jsonl",
        help="标注数据文件路径"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="../eval_results_en",
        help="评估结果文件路径"
    )
